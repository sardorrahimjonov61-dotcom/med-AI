(function initMedAI() {
  ...
  function findGuide(input) {
    ...
    return Object.entries(treatmentGuides).find(([key, guide]) => {
      if (normalized.includes(normalizeText(key))) {
        return true;
      }
      return guide.aliases.some((alias) => normalized.includes(normalizeText(alias)));
    });
  }
  ...
  function generateTreatmentText(diagnosis, complications, safetyData) {
    if (!diagnosis.trim()) {
      return 'Please enter the confirmed diagnosis first.';
    }
  ...
  diagnosisForm.addEventListener('submit', (event) => {
    ...
    if (!complaints && !vitals && !currentWorkup) {
      output.textContent = 'Please add patient complaints or exam data first.';
      return;
    }

    output.textContent = generateDifferentialText(`${complaints} ${vitals} ${currentWorkup}`);
  });
